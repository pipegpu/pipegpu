 ...
 */
const initDeferred = async (context: Context, compiler: Compiler, colorAttachments: ColorAttachment[], aspect: number, near: number, far: number): Promise<BaseHolder[]> => {
...
fn fs_main(f: FRAGMENT) -> GBUFFER {
  ...
  return o;
        ...
        // color attachments
        // position 
        const positionColorTexture = compiler.createTexture2D({
            width: context.getViewportWidth(),
            height: context.getViewportHeight(),
        ...
        const positionColorAttachment = compiler.createColorAttachment({
            texture: positionColorTexture,
            blendFormat: 'opaque',
            colorLoadStoreFormat: 'clearStore',
        });
        // normal
        const normalTexture = compiler.createTexture2D({
            width: context.getViewportWidth(),
            height: context.getViewportHeight(),
            appendixTextureUsages: GPUTextureUsage.RENDER_ATTACHMENT,
            textureFormat: 'rgba16float',
